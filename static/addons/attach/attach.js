...
(function (attach) {
    ...
    exports.attach = function (term, socket, bidirectional, buffered) {
        ...
        term._sendData = function (data) {
            socket.emit('ssh command', {data: data, room: socket.room_name});
        };

        socket.on('ssh response', term._getMessage);

        if (bidirectional) {
            term.on('data', term._sendData);
        }

        socket.on('close', term.detach.bind(term, socket));
        socket.on('error', term.detach.bind(term, socket));
    };
